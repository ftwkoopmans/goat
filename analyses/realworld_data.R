...
# output_dir = "C:/temp" # to reproduce this code on non-development system, set this to your desired output dir or `getwd()`
...
clr = tibble::tibble(
  method = c("hypergeometric", "fisherexact_ease", "idea", "idea_louis", "gsea_pvalue", "gsea_effectsize", "goat_pvalue", "goat_effectsize", "goat_pvalue_effectsize", "goat_effectsize_up", "goat_effectsize_down", "goat_effectsize_abs"),
  label = c("hypergeometric test", "Fisher-exact EASE", "iDEA", "iDEA louis", "GSEA p-value", "GSEA effectsize", "GOAT p-value", "GOAT effectsize", "GOAT p-value & effectsize", "GOAT effectsize up", "GOAT effectsize down", "GOAT effectsize abs"),
  clr = c("#69f0ae", "#ffd600", "#C99D8E", "#8d6e63", "#42a5f5", "#304ffe", "#ff7043", "#f50057", "#d50000", "#f50057", "#f50057", "#f50057")
)
...
for(n in dataset_names_go_analysis) {
  file_cache = paste0(output_dir, "/cache__", gsub("[^a-zA-Z0-9 _-]", " ", n), ".rda")
  ...
  if(file.exists(file_cache)) {
    ...
    # apply geneset enrichment algorithms. Remove columns that are not needed downstream to reduce RAM footprint of this script (because we're applying many algorithms to many datasets)
    n_result = bind_rows(
    ...
    if(include_idea) {
      time_start = Sys.time()
      tmp = test_genesets(genesets_filtered, genelist, method = "idea", return_pvalue_louis = FALSE, padj_method = padj_method, padj_cutoff = padj_cutoff)
      time_diff = as.numeric(difftime(Sys.time(), time_start, units = "secs"))
      n_result = bind_rows(n_result,
                           tmp |> select(id, source, ngenes_input, ngenes, ngenes_signif, pvalue, pvalue_adjust, signif, ismissing_idea) |> mutate(method = "idea"), # because of `return_pvalue_louis = FALSE` , columns pvalue and pvalue_idea are the same
                           tmp |> mutate(pvalue = pvalue_louis_idea) |>
                             padjust_genesets(method = padj_method, cutoff = padj_cutoff) |>
                             select(id, source, ngenes_input, ngenes, ngenes_signif, pvalue, pvalue_adjust, signif, ismissing_idea) |> mutate(method = "idea_louis")
      )
      rm(tmp)
    ...
    # monitor progress; print |signif| per method to console
    print(n_result |> filter(signif) |> count(method), n = Inf)
    ...
    hist(goat:::rankscore_fixed_order(nrow(genelist)), xlab = "Gene score", main = "")
...
tmp = results |>
  ...
  left_join(
    ...
    results |> filter(method == "idea" & ! ismissing_idea %in% TRUE) |> select(dataset_label, source, id, pvalue_idea = pvalue),
...
# compose plot data
tib_plot = bind_rows(
  tmp |> mutate(classification = "all genesets"),
  tmp |> filter(is.na(pvalue_idea)) |> mutate(classification = "missing in iDEA")
) |>
  mutate(classification = factor(classification, levels = c("all genesets", "missing in iDEA"))) # enforce plot order
...
p = ggplot(tib_plot, aes(pvalue_minlog10, fill = classification, colour = classification)) +
  ...
  labs(x = "-log10 geneset p-value @ GOAT effectsize", y = "Density") +
  theme_bw() +
  theme(
...
################################################################################################################################################################################
# scatterplot fGSEA vs iDEA
for(plotspec in list(
  list(method="idea", lbl="iDEA"),
  list(method="idea_louis", lbl="iDEA louis")
)) {
  tmp = results |>
  ...
  p = ggplot(tib_plot, aes(minlog10_fixzero(gsea_effectsize, limit = NA), minlog10_fixzero(idea, limit = NA), colour = missing_in_idea)) +
    ...
    labs(x = "-log10 pvalue @ GSEA effectsize", y = paste("-log10 pvalue @", plotspec$lbl)) +
    theme_bw() +
    theme(
      legend.position = "bottom",
      legend.key.size = unit(0.75, "line"),
...
p = ggplot(tib_plot, aes(minlog10_fixzero(gsea_effectsize, limit = NA), minlog10_fixzero(goat_effectsize, limit = NA))) +
  ...
  geom_abline(intercept = 0, slope = 1, colour = "black") +
  facet_wrap(.~dataset_label, scales = "free") +
  labs(x = "-log10 pvalue @ GSEA effectsize", y = "-log10 pvalue @ GOAT effectsize") +
  theme_bw() +
  theme(
    legend.position = "bottom",
    legend.key.size = unit(0.75, "line"),
...
mybarplot = function(x, fname) {
  ...
  print(p)
  ggsave(p, file = fname, width = 8, height = 4.5)
}

mybarplot(results, paste0(output_dir, "/realworld_barplot_signif_count - incl idea raw pvalues.pdf"))
mybarplot(results |> filter(!grepl("idea", method)), paste0(output_dir, "/realworld_barplot_signif_count - without idea.pdf"))
mybarplot(results |> filter(method != "idea"), paste0(output_dir, "/realworld_barplot_signif_count.pdf"))
...
if(include_idea) {
  ...
  tmp |>
    group_by(dataset_label) |>
    mutate(
      n_idea = n[method == "idea"],
      gain_idea = n - n[method == "idea"],
      gain_idea_pct = round((n - n[method == "idea"]) / n[method == "idea"] * 100, 1)
    ) |>    ungroup() |>
    filter(method == "goat_effectsize") |>
    print(n = Inf)
  cat("*** idea louis\n")
  tmp |>
    group_by(dataset_label) |>
    mutate(
      n_idea_louis = n[method == "idea_louis"],
      gain_idea_louis = n - n[method == "idea_louis"],
      gain_idea_louis_pct = round((n - n[method == "idea_louis"]) / n[method == "idea_louis"] * 100, 1)
    ) |>
    ungroup() |>
    filter(method == "goat_effectsize") |>
    print(n = Inf)
...
for(dataset_name in grep("Colameo 2021", unique(results$dataset_label), value = T, ignore.case = T)) {
  ...
  p = ggplot(tib_plot, aes(x = pvalue_adjust_minlog10, y = id, size = ngenes, colour = method)) +
    ...
    scale_y_discrete(breaks = levels(tib_plot$id), labels = goat:::string_trunc_right(tib_plot$name[match(levels(tib_plot$id), tib_plot$id)], width = 50)) +
