...
#' @param score type of score to show on the x-axis. Options: "minlogp" to show -log10(adjusted pvalue), which is default. Set "oddsratio" to show the enrichment of significant genes.
...
#' @export
plot_lollipop = function(x, output_dir = NA, only_reduced = FALSE, show_pvalue = FALSE, score = "minlogp", score_colorscale = "source", max_ngenes = NA, topn = NA, padj_cutoff = NA) {
  signif_and_reduced = ngenes = pvalue_adjust = pvalue = rank__ = name = score_oddsratio = score_clr = NULL # fix invisible bindings R package NOTE
  check_dependency("ggplot2", "plot lollipop chart")
  x = validate_genesets(x)
  stopifnot("parameter output_dir must be a full path for the output files, this directory must already exists" =
              length(output_dir) == 1 && (is.na(output_dir) || (is.character(output_dir) && nchar(output_dir) > 4 && dir.exists(output_dir)) ) )
  stopifnot("parameter score must be any of; 'minlogp', 'oddsratio'" = length(score) == 1 && score %in% c("minlogp", "oddsratio"))
  ...
  if(score == "oddsratio") {
    stopifnot("parameter score was set to 'oddsratio' so we expect column 'score_oddsratio' in the provided data table" = "score_oddsratio" %in% colnames(x))
    xlab = "Enrichment ratio for significant genes"
  }
  ...
  for(src in sort(unique(x$source))) {
    ...
    x_src = x |>
      filter(source == src) |>
      mutate(
        score = minlog10_fixzero(pvalue_adjust),
        name = string_trunc_right(name, width = label_nchar)
      )

    if(nrow(x_src) > 100) {
      cat(src, "has more than 100 significant genesets, not creating plot. You can run this function and explicitly configure to only show the topn most significant genesets, see documentation for 'topn' parameter\n")
      next
    }

    if(score == "oddsratio") {
      x_src = x_src |> mutate(score = score_oddsratio)
      x_src$score_clr = x_src$score
      clr_break_limits = c(0.5, 2)
      if(length(score_colorscale) == 1 && is.character(score_colorscale) && score_colorscale == "source") {
        clr_break_limits = c(min(0.5, min(x_src$score)), max(2, x_src$score))
      }
      if(length(score_colorscale) == 1 && is.character(score_colorscale) && score_colorscale == "overall") {
        clr_break_limits = c(min(0.5, min(x$score)), max(2, x$score))
      }
    } else {
      clr_break_limits = c(10^-6, 0.01)
      x_src$score_clr = x_src$pvalue_adjust
      worst_pval = max(x$pvalue_adjust, na.rm = T)
      if(length(score_colorscale) == 1 && is.character(score_colorscale) && score_colorscale == "source") {
        clr_break_limits = c(min(10^-5, min(x_src$pvalue_adjust, na.rm=T)), ifelse(worst_pval > 0.01, 0.05, 0.01))
      }
      if(length(score_colorscale) == 1 && is.character(score_colorscale) && score_colorscale == "overall") {
        clr_break_limits = c(min(10^-5, min(x$pvalue_adjust, na.rm=T)), ifelse(worst_pval > 0.01, 0.05, 0.01))
      }
    }

    if(length(score_colorscale) == 2 && is.numeric(score_colorscale)) {
      clr_break_limits = score_colorscale
    }


    # threshold colors to plot range
    x_src$score_clr[x_src$score_clr < clr_break_limits[1]] = clr_break_limits[1]
    x_src$score_clr[x_src$score_clr > clr_break_limits[2]] = clr_break_limits[2]
    ...
    # hack to always have same plot area for labels; pre-pad whitespace to fixed-width strings of 50 characters.
    # one could use stringi::stri_pad_left() or a wrapper thereof like stringr::str_pad(),
    # but instead we here use some naive vanilla R code to prevent a package dependency just for this.
    ...
    # better implementation through labeling::extended() function, which is not included here to keep external dependencies to a minimum
    myxlim = c(0, max(pretty(c(0, max(1, max(x_src$score))), n = 4)))
    ...
    p = ggplot2::ggplot(x_src, ggplot2::aes(x = score, y = id, size = ngenes, colour = score_clr)) +
      ggplot2::geom_segment(ggplot2::aes(x=0, xend=score, y=id, yend=id), colour = "darkgrey", size = 0.5) +
      ...
      ggplot2::scale_colour_viridis_c(direction = -1, alpha = 0.7, trans = "log10", limits = clr_break_limits, name = "p.adjust") +
    ...
    if(has_few_rows) {
      p = p +
        ggplot2::guides(
          colour = ggplot2::guide_colourbar(reverse = TRUE, barwidth = ggplot2::unit(4, "lines"), order = 1,
                                            title.vjust = 1, label.theme = ggplot2::element_text(angle = -90, vjust = 0.5, size = 9)),
          size = ggplot2::guide_legend(keywidth = ggplot2::unit(0.1, "lines"), keyheight = ggplot2::unit(0.1, "lines"), order = 0)
        ) +
        ggplot2::theme(legend.position = "bottom")
    } else {
      # subset of the above block
      p = p + ggplot2::guides(
        colour = ggplot2::guide_colourbar(reverse = TRUE, barheight = ggplot2::unit(4, "lines"), order = 1, label.vjust = 1),
        size = ggplot2::guide_legend(keywidth = ggplot2::unit(0.1, "lines"), keyheight = ggplot2::unit(0.1, "lines"), order = 0)
      )
    }
    ...
    p = p +
      ggplot2::theme(
        ...
        legend.spacing.y = ggplot2::unit(0.1, "lines"), # remove spacing between legend keys and items
    ...
    if(show_pvalue) {
      p = p + ggplot2::geom_text(ggplot2::aes(label = sprintf("%.1e", pvalue_adjust)), size = 2.5, colour = "black", hjust = 0, nudge_x = max(x_src$score)/33, show.legend = FALSE)
    }
    ...
    if(!is.na(output_dir)) {
      f = sprintf("%s/goat_lollipop_%s%s%s%s%s.pdf",
                  output_dir,
      ...
      if(file.exists(f)) {
        file.remove(f)
        if(file.exists(f)) {
          stop(paste("failed to overwrite existing file when creating a lollipop plot:", f))
        }
      }
