...
#' This function applies the specified pvalue adjustment procedure within each 'source' in the genesets table and finally corrects p-values for the number of different geneset sources/databases tested.
...
#' @param padj_method method for p-value adjustment, passed to `stats::p.adjust()` via `padjust_genesets()`, e.g. set "BH" to compute FDR adjusted p-values (default) or "bonferroni" for a more stringent procedure
...
#' @export
test_genesets = function(genesets, genelist, method, padj_method = "BH", padj_cutoff = 0.01, padj_min_signifgenes = 0L, ...) {
  ...
  settings = sprintf("test_genesets(method='%s', padj_method='%s', padj_cutoff=%s, padj_min_signifgenes=%s%s)",
                     method, padj_method, padj_cutoff, padj_min_signifgenes, ifelse(settings... == "", "", paste0(", ", settings...)))
